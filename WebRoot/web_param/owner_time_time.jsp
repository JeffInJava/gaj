...
%><option value=>所有归属方</option> <%
ResultSet rs=connora.executeQuery("select distinct bank_acc_name,owner_id from tbl_metro_acc_info where trim(acc_flag)=2 order by owner_id");
while(rs.next())
{
%><option value=<%=rs.getString("owner_id")%>>(<%=rs.getString("owner_id")%>)<%=rs.getString("bank_acc_name")%></option><%
}
}else{
	ResultSet rs=connora.executeQuery("select distinct bank_acc_name,owner_id from tbl_metro_acc_info where trim(acc_flag)=2 and substr(owner_id,1,2)='"+ (String)request.getParameter("line_id")+"' order by owner_id");
	while(rs.next())
{
%>
      <option value=<%=rs.getString("owner_id")%>>(<%=rs.getString("owner_id")%>)<%=rs.getString("bank_acc_name")%></option>
...
<td><input type=text name="start_date" style="PADDING-LEFT: 0px; FONT-WEIGHT: normal; FONT-SIZE: 12px; VERTICAL-ALIGN: middle; WIDTH: 150px; COLOR: #000000; FONT-STYLE: normal; FONT-FAMILY: 宋体; HEIGHT: 20px; BACKGROUND-COLOR: #ffffff; TEXT-ALIGN: left; TEXT-DECORATION: none" ondblclick="CalendarWebControl.show(this,false,this.value);"></td>
