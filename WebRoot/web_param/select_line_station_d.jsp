...
<%
String[] line=(String[])session.getAttribute("line_d");
String[] line_station=(String[])session.getAttribute("line_station");
int line_count=0;
int line_flag=line_count;
if(line_flag==0){
...
%>where[<%=i%>]=new comefrom("<%=line[i]%>","<%=line_station[i]%>");<%
}
session.setAttribute("line_d",line);
...
function select_o() {
	with(document.form1.line_select_o) {
		...
		for(i = 0;i < where.length;i ++){
			if (where[i].line == loca2) {
				...
				for(j = 0;j < loca3.length;j++) {
					with(document.form1.station_select_o) {
					length = loca3.length;
...
function select_d() {
	with(document.form1.line_select_d) {
		var loca2=options[selectedIndex].value;
	 }
		for(i = 0;i < where.length;i ++){
			if (where[i].line == loca2) {
				loca3 = (where[i].station).split("|");
				for(j = 0;j < loca3.length;j++) {
					with(document.form1.station_select_d) {
					length = loca3.length;
						options[j].text = loca3[j]; 
						options[j].value = loca3[j];
						var loca4=options[selectedIndex].value;
